...
local Library = {
	...
	ShadowColor = Color3.fromRGB(0, 180, 140);
	ShadowSize = 12;
	ShadowOffsetX = 0;
	ShadowOffsetY = 0;
...
function Library:UpdateShadows()
	for _, Data in next, Library.ShadowRegistry do
		if Data.Sync then
			Data.Sync();
		end;
	end;
...
function Library:AddShadow(Outer)
	if Library.ShadowRegistry[Outer] then
		return Library.ShadowRegistry[Outer];
	end;
	...
	local Holder = Library:Create('Frame', {
		...
		ZIndex = math.max((Outer.ZIndex or 1) - 1, 0);
		Parent = Outer.Parent;
	});
	...
	local Stroke = Library:Create('UIStroke', {
		...
		Color = Library.ShadowColor;
		Thickness = Library.ShadowSize;
		Transparency = 0.35;
		Parent = Glow;
	});
	...
	local function Sync()
		local spread = math.max(0, Library.ShadowSize or 0);
		local offsetX = Library.ShadowOffsetX or 0;
		local offsetY = Library.ShadowOffsetY or 0;

		Holder.AnchorPoint = Outer.AnchorPoint;
		Holder.Size = UDim2.new(
			Outer.Size.X.Scale,
			Outer.Size.X.Offset + spread * 2,
			Outer.Size.Y.Scale,
			Outer.Size.Y.Offset + spread * 2
		);
		Holder.Position = UDim2.new(
			Outer.Position.X.Scale,
			Outer.Position.X.Offset - spread + offsetX,
			Outer.Position.Y.Scale,
			Outer.Position.Y.Offset - spread + offsetY
		);
		if Outer.Parent then
			Holder.Parent = Outer.Parent;
		else
			Holder.Parent = nil;
		end;

		Holder.Visible = Outer.Visible and spread > 0 and Outer.Parent ~= nil;
		Stroke.Thickness = spread;
	end;

	for _, Property in next, { 'Position', 'Size', 'Visible', 'AnchorPoint', 'Parent' } do
		Outer:GetPropertyChangedSignal(Property):Connect(Sync);
	end;
	...
	Library.ShadowRegistry[Outer] = Data;
	Sync();
...
local ThemeManager = {} do
	...
	function ThemeManager:ApplyTheme(theme)
		...
		local scheme = data[2]
		for idx, val in next, customThemeData or scheme do
			if idx == 'ShadowSize' or idx == 'ShadowOffsetX' or idx == 'ShadowOffsetY' then
				self.Library[idx] = tonumber(val) or self.Library[idx]

				if Options[idx] then
					Options[idx]:SetValue(tostring(self.Library[idx]))
				end
			else
	...
	function ThemeManager:ThemeUpdate()
		...
		local options = { "FontColor", "MainColor", "AccentColor", "BackgroundColor", "OutlineColor", "ShadowColor" }
		...
		for _, field in next, { "ShadowSize", "ShadowOffsetX", "ShadowOffsetY" } do
			if Options and Options[field] then
				self.Library[field] = tonumber(Options[field].Value) or self.Library[field]
			end
		end
	...
	function ThemeManager:CreateThemeManager(groupbox)
		...
		groupbox:AddLabel('Shadow color'):AddColorPicker('ShadowColor', { Default = self.Library.ShadowColor });
		groupbox:AddSlider('ShadowSize', {
			Text = 'Shadow size';
			Default = self.Library.ShadowSize;
			Min = 0;
			Max = 40;
			Rounding = 0;
		});
		groupbox:AddSlider('ShadowOffsetX', {
			Text = 'Shadow offset X';
			Default = self.Library.ShadowOffsetX;
			Min = -40;
			Max = 40;
			Rounding = 0;
		});
		groupbox:AddSlider('ShadowOffsetY', {
			Text = 'Shadow offset Y';
			Default = self.Library.ShadowOffsetY;
			Min = -40;
			Max = 40;
			Rounding = 0;
		});
		...
		Options.ShadowColor:OnChanged(UpdateTheme)
		Options.ShadowSize:OnChanged(UpdateTheme)
		Options.ShadowOffsetX:OnChanged(UpdateTheme)
		Options.ShadowOffsetY:OnChanged(UpdateTheme)
	...
	function ThemeManager:SaveCustomTheme(file)
		...
		local theme = {}
		local fields = { "FontColor", "MainColor", "AccentColor", "BackgroundColor", "OutlineColor", "ShadowColor" }

		for _, field in next, fields do
			theme[field] = Options[field].Value:ToHex()
		end

		theme.ShadowSize = Options.ShadowSize.Value
		theme.ShadowOffsetX = Options.ShadowOffsetX.Value
		theme.ShadowOffsetY = Options.ShadowOffsetY.Value
...
local SaveManager = {} do
	...
	function SaveManager:IgnoreThemeSettings()
		self:SetIgnoreIndexes({ 
			"BackgroundColor", "MainColor", "AccentColor", "OutlineColor", "FontColor", "ShadowColor", "ShadowSize", "ShadowOffsetX", "ShadowOffsetY", -- themes
